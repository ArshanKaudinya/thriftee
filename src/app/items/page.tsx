  ...
  useEffect(() => {
    const fetchItem = async () => {
      ...
      if (data) {
        ...
        const { data: userData } = await supabase.from('users').select('date_joined').eq('id', data.user_id).maybeSingle()
  ...
  return (
    <div className="min-h-screen p-6 bg-background text-text">
      <div className="max-w-5xl mx-auto bg-surface border border-subtext rounded-xl shadow p-6 flex flex-col md:flex-row gap-6">
        <div className="relative w-full md:w-1/2 aspect-video md:aspect-square overflow-hidden" onTouchStart={handleSwipe}>
